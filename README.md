...
- **Idle auto-disconnect** — optionally have a shared-vault browse **disconnect itself** after a period of no activity (**Settings → Secure sharing → Disconnect when idle**: 5 / 15 / 30 / 60 minutes; off by default). On timeout it signals the owner, leaves the share, and securely wipes the downloaded temp copies — so an unattended session doesn't stay connected.
