...
- **Access log** — the owner's relay records **who opened which file, when, and for how long** (open + close are paired into a duration). The relay stores only **opaque object ids**; the real filenames are resolved **locally** on the owner's machine, so the relay never learns them. Reveal it in the hub with the **Ctrl+Alt+L** sequence. The log viewer **updates live** (it polls every few seconds and tails to the newest entry), file names are **clickable** to open them in this window, **hovering** an image entry shows a thumbnail preview, and **Export…** saves the whole log to a `.txt` file. **Clear log** wipes it on the relay.
