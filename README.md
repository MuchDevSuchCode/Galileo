...
- **Live updates** — the open folder refreshes automatically when files change on disk (downloads, other apps, etc.) — new items appear in the correct sorted position without a manual refresh, and your selection is preserved. (Uses a file-system watcher; some network/WSL shares don't emit change events — press **F5** there.)
