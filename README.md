...
- **Per-vault sharing** — while a vault is unlocked, **Share "<vault>" with friends…** toggles which linked friends can see it. Revoking takes effect live.
- **Browse with Ctrl+Alt+V** — press **Ctrl+Alt+V**, enter **your sharing passphrase**, and the vaults friends currently share with you appear — no local vault needed. Click a file to view it in-app (read-only); nothing is written except a temp copy used to open it.
