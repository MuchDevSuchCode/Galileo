...
- **Convert images** — right-click an image → **Convert → JPG to PNG** (or **PNG to JPG**) to re-encode it alongside the original (the source is left untouched). Select several images first to convert them in one go. EXIF orientation is baked into the new file; since JPEG has no transparency, converting a transparent PNG to JPG flattens those areas.
...
- **Per-vault sharing** — inside an unlocked vault, click the **Share** icon in the command bar and toggle **which friends** get it (not everyone, not everything). Revoking takes effect live.
...
- **Access log** — the owner's relay records **who opened which file, when, and for how long** (open + close are paired into a duration). The relay stores only **opaque object ids**; the real filenames are resolved **locally** on the owner's machine, so the relay never learns them. Reveal it in the hub with the **Ctrl+Alt+L** sequence.
