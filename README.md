...
- **Secure delete (overwrite wipe)** — emptying the bin (and **Shift+Delete** / right-click **Secure delete (shred)…**) **overwrites the file's bytes before deleting** so they can't be recovered, instead of a metadata-only delete. Choose the method in **Settings → Secure Delete**: **None** (plain delete), **Zero** (1 pass), **Random** (1 pass), **DoD 5220.22-M** (3 passes), **DoD ECE** (7 passes), or **Gutmann** (35 passes) — modeled on [fileshredder.org](https://www.fileshredder.org/). More passes are slower on large files (run on a background thread with status). **Caveat:** overwriting is **best-effort** — on SSDs/NVMe and copy-on-write drives, wear-leveling and TRIM mean the original bytes may survive; it is not a forensic guarantee.
