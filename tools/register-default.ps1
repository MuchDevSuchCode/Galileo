<#
.SYNOPSIS
    Registers Galileo with Windows as a selectable photo app (per-user, no admin).

.DESCRIPTION
    Creates a ProgID, an "Applications" entry (for the Open with list), per-extension
    OpenWithProgids entries, and a Capabilities/RegisteredApplications entry (so Galileo
    ...
    this, set Galileo as the default in Settings > Default apps (or right-click a photo >
    Open with > Choose another app > Always).

.PARAMETER ExePath
    Full path to Galileo.App.exe. Defaults to the published install under %LocalAppData%.
#>
param(
    [string]$ExePath = (Join-Path $env:LOCALAPPDATA 'Galileo\app\Galileo.exe')
)

$ErrorActionPreference = 'Stop'

if (-not (Test-Path $ExePath)) {
    Write-Error "Galileo.App.exe not found at: $ExePath`nPublish first, or pass -ExePath."
    return
}
...
Write-Host "Registering Galileo -> $ExePath" -ForegroundColor Cyan
...
$cap = 'HKCU:\Software\Galileo\Capabilities'
Ensure-Key $cap
...
foreach ($ext in $exts) {
    $dot = ".$ext"
    # add Galileo to the Open-with list for this extension (does not steal the default)
...
Set-ItemProperty 'HKCU:\Software\RegisteredApplications' -Name 'Galileo' -Value 'Software\Galileo\Capabilities'
...
$shell = Add-Type -MemberDefinition $sig -Namespace 'GalileoWin32' -Name 'Shell' -PassThru
